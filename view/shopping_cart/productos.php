...
if($action=='added'){
    echo "<div class='alert alert-info'>";
        echo "<strong>{$nombre}</strong> ¡agregado a tu carrito!";
    echo "</div>";
}
 
else if($action=='failed'){
    echo "<div class='alert alert-info'>";
        echo "<strong>{$nombre}</strong> No se pudo agregar a su carrito!";
    echo "</div>";
}
 
// seleccionar productos de la bd
$query = "SELECT p.id, p.nombre, p.url_image, p.precio, ci.quantity 
        FROM producto p 
